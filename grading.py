...
class Grades:
  ...
  def __init__(self, projectName, questionsAndMaxesList,
               gsOutput=False, edxOutput=False, muteOutput=False, gradingOutpath = None):
    """
    ...
    self.gradingOutpath = gradingOutpath
    self.gsOutput = gsOutput  # GradeScope output
    self.mute = muteOutput
    self.prereqs = defaultdict(set)

    self.custom_grade =  dict([(q, []) for q in self.questions])
    """
      {
        "q1":[
                [message_for_testcase1,bool_for_testcase1,#expanded_for_testcase1, time_for_testcase1],
                [message_for_testcase2,bool_for_testcase2,#expanded_for_testcase2, time_for_testcase2],
                ...
              ],
        "q2":...
      }
    ...
    """
      Make sure there is only 1 test case in each question for actual grading
    """
    #print('Autograder transcript for %s' % self.project)
    print('Starting on %d-%d at %d:%02d:%02d' % self.start)

  def custom_assignGrade(self,data):
    """
      data is supposed to be message, pass_bool, nodes_expanded for a particular test case
    """
    self.custom_grade[self.currentQuestion].append(data)
  def custom_writeLog(self):
    outfile = open(self.gradingOutpath,'w')
    for question in self.custom_grade:
      for test_case in range(len(self.custom_grade[question])):
        datum = ""
        for val in self.custom_grade[question][test_case]:
          datum+=","+str(val)
        print(str(question)+datum,file = outfile)
    outfile.close()
...
""")
    ...
    if self.gradingOutpath:
      self.custom_writeLog()
  ...
  def produceGradeScopeOutput(self):
    ...
    with open('gradescope_response.json', 'w') as outfile:
        json.dump(out_dct, outfile, indent = 2)
    return
