...
def readCommand(argv):
    ...
    parser.add_option('--grading_outpath',
                    dest = 'gradingOutpath',
                    default = None,
                    help = 'Output path of grades to be written')
...
def evaluate(generateSolutions, testRoot, moduleDict, exceptionMap=ERROR_HINT_MAP,
            ...
            printTestCase=False, questionToGrade=None, display=None, gradingOutpath = None):
        ...
        for t in sorted(tests):
            ...
            def makefun(testCase, solution_file):
                if generateSolutions:
                    ...
                    return lambda grades,custom_result: testCase.writeSolution(moduleDict, solution_file, result=custom_result)
                else:
                    # read in solution dictionary and pass as an argument
                    testDict = testParser.TestParser(test_file).parse()
                    solutionDict = testParser.TestParser(solution_file).parse()
                    if printTestCase:
                        return lambda grades,custom_result: printTest(testDict, solutionDict) or testCase.execute(grades, moduleDict, solutionDict, result=custom_result)
                    else:
                        return lambda grades,custom_result: testCase.execute(grades, moduleDict, solutionDict, custom_result)
    ...
    grades = grading.Grades(projectParams.PROJECT_NAME, questions,
                            gsOutput=gsOutput, edxOutput=edxOutput, muteOutput=muteOutput, gradingOutpath = gradingOutpath)
    if questionToGrade == None:
...
if __name__ == '__main__':
    ...
    if options.runTest != None:
        ...
        evaluate(options.generateSolutions, options.testRoot, moduleDict,
            ...
            gradingOutpath = options.gradingOutpath)
