...
class CustomCreditQuestion(Question):
    def execute(self, grades):
        # grades.assignZeroCredit()
        all_pass = True
        for _, f in self.testCases:
            _result = []  # this would be inplace filled with the custom result we want
            f(grades, custom_result = _result)
            grades.custom_assignGrade(_result)
            all_pass = (all_pass and _result[1])
